	...
	foreach($order_ids as $order_id){
		...
		if($sent_zoom_link!=1){
			...
			try {
				...
				error_log('[TWWT] Zoom email failed for order ' . $order_id . ': ' . $e->getMessage());
			}

			$smscontent = isset($settings['sms_zoom_notification']) ? $settings['sms_zoom_notification'] : '';
			if ( ! empty( $smscontent ) && ! empty( $phone ) ) {
				...
				try {
					...
					error_log('[TWWT] Zoom SMS failed for order ' . $order_id . ': ' . $e->getMessage());
				}
			}
...
function twwt_npo_email_send($email, $first_name, $product_name, $url){
	...
	echo wpautop($emailcontent);
	wc_get_template( 'emails/email-footer.php' );
	echo '<p style="text-align:center;margin-top:0;">If you would prefer not receiving our emails Please <a href="'.esc_url(twwt_get_unsubscribe_url($email)).'">unsubscribe</a></p>';
	$message = ob_get_clean();
	$subject = $email_heading;
	return $mailer->send( $email, $subject, $message);
	//return true;
...
//subscribe
function twwt_generate_unsubscribe_token($email) {
    return hash_hmac('sha256', strtolower(trim($email)), wp_salt('auth'));
}

function twwt_get_unsubscribe_url($email) {
    $token = twwt_generate_unsubscribe_token($email);
    return home_url('/?unsub=email&email=' . urlencode($email) . '&token=' . urlencode($token));
}

add_action('init', 'mt_unsubscribe');
function mt_unsubscribe(){
    if( isset($_GET['unsub']) && $_GET['unsub']=="email"){
        $email = isset($_GET['email']) ? sanitize_email($_GET['email']) : '';
        $token = isset($_GET['token']) ? sanitize_text_field($_GET['token']) : '';

        if ( empty($email) || empty($token) ) {
            wp_die('Invalid unsubscribe request.', 'Error', array('response' => 400));
        }

        $expected_token = twwt_generate_unsubscribe_token($email);
        if ( ! hash_equals($expected_token, $token) ) {
            wp_die('Invalid unsubscribe token.', 'Error', array('response' => 403));
        }
